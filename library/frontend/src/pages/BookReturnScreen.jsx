...
export default function BookReturnScreen() {
    ...
    const [borrowingInfoError, setBorrowingInfoError] = useState('')
    ...
    const fetchBorrowingInfo = async () => {
        try {
            ...
            if (!response.ok) {
                ...
                setBorrowingInfoError(
                    typeof data?.detail === 'string'
                        ? data.detail
                        : 'Không tải được danh sách sách đang mượn'
                )
                return
            }

            const borrowedBooks = Array.isArray(data?.borrowed_books) ? data.borrowed_books : []
            setBorrowingInfo({
                ...data,
                borrowed_books: borrowedBooks,
                currently_borrowed: typeof data?.currently_borrowed === 'number' ? data.currently_borrowed : borrowedBooks.length,
                max_books: typeof data?.max_books === 'number' ? data.max_books : 0,
                fine_balance: typeof data?.fine_balance === 'number' ? data.fine_balance : 0
            })
            setBorrowingInfoError('')
        } catch (err) {
            ...
            setBorrowingInfoError('Lỗi kết nối khi tải danh sách sách đang mượn')
        }
    }
    ...
    const borrowedBooks = Array.isArray(borrowingInfo?.borrowed_books) ? borrowingInfo.borrowed_books : []
        ...
        <div className="book-return-screen">
            ...
            <main className="book-content-layout">
                ...
                <aside className="active-borrowings-panel">
                    ...
                    {borrowingInfoError && (
                        <div className="loading-small" style={{ marginTop: 8, opacity: 0.9 }}>
                            {borrowingInfoError}
                        </div>
                    )}
                </aside>
